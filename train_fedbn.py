...
train_fedbn.py
--------------
FedBN simulation over PCMMD clients.

FedBN keeps per-client batch-norm statistics local (not aggregated), which
handles feature-level heterogeneity without any architecture change.

Reference: Li et al. (2021) FedBN: Federated Learning on Non-IID Features via
Local Batch Normalization. ICLR 2021.

Outputs:
  results/fedbn_round_logs.csv
  results/fedbn_results.csv
  results/communication_analysis.csv    (appended)
  results/client_analysis.csv           (appended)
  figures/fedbn/{f1,loss,pr_auc}_curve_<dist>.png
  figures/clients/client_perf_fedbn_<dist>.png
  checkpoints/fedbn/<backbone>_<dist>_fold<k>.pth

Run:  python train_fedbn.py --config configs/fedbn.yaml [--distribution iid] [--fold 2]
"""
...
from fl.fedbn import aggregate_fedbn  # noqa: E402
...
def run_fedbn(cfg, device, logger) -> None:
    ...
    logger.info("FedBN | dist=%s partition=%s clients=%d C=%.2f E=%d rounds=%d fold=%d",
    ...
    ckpt_dir = Path(cfg["ckpt_dir"]) / "fedbn"; ckpt_dir.mkdir(parents=True, exist_ok=True)
    ...
    for rnd in range(1, cfg["num_rounds"] + 1):
        ...
        # FedBN: aggregate all params except BN running stats
        global_model = aggregate_fedbn(global_model, client_weights, client_sizes)

        vm = evaluate_with_loss(global_model, monitor, criterion, device)
        elapsed = time.time() - t0
        cumulative_mb += model_mb * k_selected * 2
        logger.info("[r%03d] clients=%d/%d train_loss=%.4f val_f1=%.4f val_auc=%.4f (%.1fs)",
                    rnd, k_selected, n_clients, round_loss / max(1, sum(client_sizes)), vm["f1"], vm["roc_auc"], elapsed)
        round_logs.append({"method": "fedbn", "distribution": dist_label, "fold": fold_id,
        ...
        comm_logs.append({"method": "fedbn", "distribution": dist_label, "fold": fold_id, "round": rnd,
    ...
    append_csv(Path(cfg["results_dir"]) / "fedbn_round_logs.csv", rl,
    ...
    summary = pd.DataFrame([{"method": "fedbn", "distribution": dist_label, "fold": fold_id,
    ...
    append_csv(Path(cfg["results_dir"]) / "fedbn_results.csv", summary,
               subset=["method", "distribution", "fold", "backbone"])

    cdf = per_client_metrics(global_model, client_loaders, device, diag_map)
    cdf.insert(0, "method", "fedbn"); cdf.insert(1, "distribution", dist_label); cdf.insert(2, "fold", fold_id)
    ...
    plot_client_performance(cdf["patient_id"], cdf["f1"],
                            Path(cfg["figures_dir"]) / "clients" / f"client_perf_fedbn_{dist_label}.png",
                            ylabel="F1", title=f"FedBN {dist_label} per-client F1", colors=colors)

    fig_dir = Path(cfg["figures_dir"]) / "fedbn"; fig_dir.mkdir(parents=True, exist_ok=True)
    for metric, col in [("f1", "val_f1"), ("loss", "val_loss"), ("pr_auc", "val_pr_auc")]:
        if col not in rl:
            continue
        fig, ax = plt.subplots(figsize=(6, 4))
        ax.plot(rl["round"], rl[col], marker="o", ms=3)
        ax.set_xlabel("Round"); ax.set_ylabel(metric); ax.set_title(f"FedBN {dist_label} — {metric}")
...
def main() -> None:
    ...
    ap.add_argument("--config", default="configs/fedbn.yaml")
    ...
    set_seed()
    device = get_device()
    logger = setup_logging(Path(cfg["results_dir"]) / "logs" / "fedbn.log")
    save_config_snapshot(cfg, Path(cfg["results_dir"]) / "configs" / "fedbn.json")
    run_fedbn(cfg, device, logger)
