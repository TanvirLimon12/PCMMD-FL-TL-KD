...
def run_one_mu(cfg, device, logger, client_loaders, monitor, test_loader, n_clients, dist_label, mu):
    ...
    for rnd in range(1, cfg["num_rounds"] + 1):
        ...
        for loader in client_loaders.values():
            ...
            res = FedProxClient(local, loader, criterion, opt, device, mu=mu).train(epochs=cfg["local_epochs"])
        ...
        logger.info("[mu=%s r%03d] train_loss=%.4f val_loss=%.4f val_f1=%.4f (%.1fs)",
                    mu, rnd, round_loss / max(1, sum(client_sizes)), vm["loss"], vm["f1"], elapsed)
        round_logs.append({"method": "fedprox", "distribution": dist_label, "mu": mu, "round": rnd,
                           "train_loss": round(round_loss / max(1, sum(client_sizes)), 5),
                           "val_loss": round(vm["loss"], 5),
                           **{f"val_{k}": round(vm[k], 5) for k in METRIC_COLS}})
        comm_logs.append({"method": "fedprox", "distribution": dist_label, "round": rnd, "mu": mu,
...
def main() -> None:
    ...
    logger.info("FedProx | dist=%s clients=%d mu_values=%s rounds=%d val_patients=%s",
                dist_label, n_clients, mu_values, cfg["num_rounds"], val_pat)
    ...
    for mu in mu_values:
        ...
        append_csv(Path(cfg["results_dir"]) / "communication_analysis.csv", cl,
                   subset=["method", "distribution", "mu", "round"])
    ...
    append_csv(Path(cfg["results_dir"]) / "fedprox_round_logs.csv", rounds_df,
               subset=["method", "distribution", "mu", "round"])
    append_csv(Path(cfg["results_dir"]) / "fedprox_results.csv", pd.DataFrame(summaries),
    ...
    if best_overall[1] is not None:
        ...
        cdf.insert(0, "method", "fedprox"); cdf.insert(1, "distribution", dist_label)
